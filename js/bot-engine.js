...
window.createBotEngine = function(level = 'medium', options = {}) {
    ...
    return {
        ...
        async getBestMoveWithEval(fen, options = {}) {
            ...
            const isRetriableInitError = (error) => {
                const message = String(error?.message || error || '');
                return (
                    message.includes('ready timeout') ||
                    message.includes('engine init failed') ||
                    message.includes('worker script not found') ||
                    message.includes('wasm not found') ||
                    message.includes('Bot worker is not initialized')
                );
            };
            ...
            for (let attempt = 0; attempt < 2; attempt += 1) {
                try {
                    ...
                    if (attempt === 1 || !isRetriableInitError(error)) {
                        throw error;
                    }
                    console.warn('engine init failed: retrying getBestMoveWithEval once', error);
                    hardResetEngineState({ error });
                }
            }
